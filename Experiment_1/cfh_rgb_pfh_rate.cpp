...
void CFH_Estimation_RGB_PFH_RATE::computeFeature(pcl::PointCloud<pcl::FPFHSignature33>& output)
{
    //resize output
...
void CFH_Estimation_RGB_PFH_RATE::setInputCloud(pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_cloud) { input_cloud_ = input_cloud; }
void CFH_Estimation_RGB_PFH_RATE::setInputNormal(pcl::PointCloud<pcl::Normal>::Ptr input_normals) { input_normals_ = input_normals; }
void CFH_Estimation_RGB_PFH_RATE::setInputKeypoints(pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_keypoints) { input_keypoints_ = input_keypoints; }
void CFH_Estimation_RGB_PFH_RATE::setSearchRadius(double search_radius) { search_radius_ = search_radius; }

void CFH_Estimation_RGB_PFH_RATE::searchForNeighbors(std::size_t index, double radius, pcl::Indices& indices, std::vector<float>& distances)
{
    ...
    {
        PCL_WARN("CFH_Estimation_RGB_PFH_RATE::searchForNeighbors: kdtree.radiusSearch() == 0");
    }

}

void CFH_Estimation_RGB_PFH_RATE::computePointPFHRGBSignature(pcl::PointCloud<pcl::PointXYZRGB>& cloud,pcl::Indices& indices, int nr_split, Eigen::VectorXf& pfhrgb_histogram)
{
    int h_index;
...
bool CFH_Estimation_RGB_PFH_RATE::computeRGBPairFeatures(pcl::PointCloud<pcl::PointXYZRGB>& cloud,int p_idx, int q_idx, float& f1, float& f2, float& f3)
{
...
bool CFH_Estimation_RGB_PFH_RATE::computeRGBPairFeatures(const Eigen::Vector4i& colors1,const Eigen::Vector4i& colors2, float& f1, float& f2, float& f3)
{
